...
END;
	...
	foreach ($ids as $id) {
		...
		echo "    <bmdb:Title>".escapeEntities($b->getTitle())."</bmdb:Title>\r\n";
		echo "    <bmdb:ShortName>".escapeEntities($b->getShortName())."</bmdb:ShortName>\r\n";
		...
		echo "    <bmdb:Description>".escapeEntities($b->getDescription())."</bmdb:Description>\r\n";
		...
		if (count($b->getStudies()) > 0) {
			...
			foreach ($b->getStudies() as $study) {
				echo "      <bmdb:Study rdf:resource=\"http://bmdb.jpl.nasa.gov/edit/study/?id={$study->getObjId()}\"/>\r\n";
			}
			echo "    </bmdb:Studies>\r\n";
		} else {
			echo "    <bmdb:Studies/>\r\n";
