...
<h5 id="urn">urn:edrn:study:<?php echo $study['Study']['FHCRC_ID']?></h5>
	<div id="smalllinks">
		<ul>
		  <li class="activeLink"><a href="/studies/view/<?php echo $study['Study']['id']?>">Basics</a></li>
		  <li class=""><a href="/studies/publications/<?php echo $study['Study']['id']?>">Publications</a></li>
		  <li class=""><a href="/studies/resources/<?php echo $study['Study']['id']?>">Resources</a></li>
		</ul>
		<div class="clr"><!--  --></div>
	</div>
		<table class="editable_info" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<table style="width:100%;">
				  <tr>
					...
					<?php foreach ($sites as $site) : ?>
					  ...
					  <td style="border:none;padding:0;padding-left:10px;margin:0;"><a style="color:#d55;font-size:90%;" alt="Remove" href="/studies/removeSite/<?php echo "{$site['sites']['site_id']}"?>/<?php echo "{$study['Study']['FHCRC_ID']}"?>/<?php echo "{$study['Study']['id']}"?>">x Remove</a></td>
					  </tr>
					<?php endforeach;?>
				      </table>
				    </td>
				...
				<div id="site-add" class="fadeOut">
				  <form action="/studies/addSite" method="POST">
			...
			<tr>
				...
				<table style="width:100%;">
				  <tr>
					...
					<?php foreach ($people as $person) : ?>
					  ...
					  <td style="border:none;padding:0;padding-left:10px;margin:0;"><a style="color:#d55;font-size:90%;" alt="Remove" href="/studies/removeResearcher/<?php echo "{$person['person_study']['person_id']}"?>/<?php echo "{$person['person_study']['study_id']}"?>">x Remove</a></td>
					  </tr>
					<?php endforeach;?>
				      </table>
				    </td>
				...
				<div id="person-add" class="fadeOut">
				  <form action="/studies/addResearcher" method="POST">
...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
    new eip($$('.editable'), '/studies/savefield', {action: 'update'});
    new eiplist($$('.editablelist'),'/studies/savefield', {action: 'update'});
  });
...
jQuery('.fakelink').each(function(index){
  ...
  jQuery('#researcher-search').each(function() {
    jQuery(this).autocomplete({
      source: '/studies/getResearcherList',
      select: function(event, ul) {
        var person = ul.item.value.split('|')[0];
        var id = ul.item.value.split('|')[1];
