<?php
	...
	echo $this->Html->css('biomarkers-organs');
?>
...
<div id="smalllinks">
		<ul>
		  ...
		  <li class="activeLink"><a href="/biomarkers/organs/<?php echo $biomarker['Biomarker']['id']?>">Organs</a></li>
...
<div id="smallEditlinks">
	<ul>
		<?php foreach($organdatas as $od): ?>
			...
			<a href="/biomarkers/organs/<?php echo $biomarker['Biomarker']['id'] . "/" . $od['OrganData']['id']?>">
...
<h4 class="organDataBlockHeader">Associated 
		<div class="editlink">
			<a href="/biomarkers/removeOrganData/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>">
				<span class="biomarkerLightRed">x Delete</span>
			</a>
			&nbsp;| 
			<a href="/acls/edit/biomarkerorgan/<?php echo $organData['OrganData']['id']?>/<?php echo $biomarker['Biomarker']['id']?>">
				Set Security
			</a>
		</div>
	...
	<div class="organdatablock">
		...
		<div id="adddefinition" class="addstudydata fadeOut">
			...
			<div style="width:80%;">
				<form action="/biomarkers/addOrganTermDefinition" method="POST">
					...
					<span class="hint">Can't find your term? <a href="/terms/define">Define It!</a></span><br/>
		...
		<?php foreach ($organData['Term'] as $term):?>
			<dt>
				<?php echo $term['label']?> &nbsp;&nbsp;
				<a class="definitionTermRemove" alt="Remove" href="/biomarkers/removeOrganTermDefinition/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $term['id']?>">
					x Remove Definition
				</a>
			</dt>
		...
		<div id="addstudydata" class="addstudydata fadeOut">
			...
			<div style="width:80%;">
				<form action="/biomarkers/addorganstudydata" method="POST">
					<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
					<input type="hidden" name="biomarker_id"  value="<?php echo $biomarker['Biomarker']['id']?>"/>
		...
		<div>
			<a class="studyAssociationRemove" href="/biomarkers/removeOrganStudyData/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $study['id']?>" onclick="return confirm('This action can not be undone. Continue?')">
		...
		<!-- start study  -->
					<div class="lefttext studyDataLeftColumn">
						<span id="description" class="textarea">
							<?php Biomarker::printor(substr($study['Study']['studyAbstract'],0,600).'&nbsp;<a href="/studies/view/'.$study['Study']['id'].'" style="text-decoration:underline;font-size:90%;"><em>Click here to read more about this study</em></a>','<em>No Description Provided Yet.</em>');?>
						</span>
					</div>
					<!-- RELEVANT STUDY DATA -->
					<div class="rightcol" id="studyDataParameters">
						...
						<br/>
						<a class="studyDefinitionLink" href="/studies/view/<?php echo $study['Study']['id']?>">
					...
					<div id="addsensspec<?php echo $study['id']?>" class="addstudypub fadeOut" style="margin-left:14px;">
						...
						<form class="studyDetailsEditForm" action="/biomarkers/addsensspec" method="POST">
							<input type="hidden" name="biomarker_id" value="<?php echo $biomarker['Biomarker']['id']?>"/>
							<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
						...
						?>
							<tr><td><?php echo $s['notes']?></td>
								...
								<td style="text-align:center;">
									<a class="fakelink" href="/biomarkers/editsensspec/<?php echo $s['id']?>/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $study['id']?>">Edit</a>
								</td>
								<td style="text-align:center;">
									<a href="/biomarkers/removesensspec/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $study['id']?>/<?php echo $s['id']?>" 
										style="color:#d55;" 
					...
					<div id="addstudypub<?php echo $study['id']?>" class="addstudypub fadeOut" style="margin-left:14px;">
						...
						<form action="/biomarkers/addstudydatapub" method="POST">
							<input type="hidden" name="biomarker_id"  value="<?php echo $biomarker['Biomarker']['id']?>"/>
							<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
							<input type="hidden" name="study_data_id" value="<?php echo $study['id']?>"/>
						...
						<?php foreach ($study['Publication'] as $publication):?>
							<li>
								<div class="studypubsnippet">
									<a href="/publications/view/<?php echo $publication['id']?>">
										<?php echo $publication['title']?>
									</a> 
									&nbsp;[
									<a href="/biomarkers/removeStudyDataPub/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $study['id']?>/<?php echo $publication['id']?>">
					...
					<div id="addstudyres<?php echo $study['id']?>" class="addstudyres fadeOut" style="margin-left:14px;">
						...
						<form action="/biomarkers/addStudyDataResource" method="POST" style="margin-top:5px;">
							<input type="hidden" name="biomarker_id"  value="<?php echo $biomarker['Biomarker']['id']?>"/>
							<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
							<input type="hidden" name="study_data_id" value="<?php echo $study['id']?>"/>
					...
					<?php foreach ($study['StudyDataResource'] as $resource):?>
						<li><div class="studyressnippet">
								...
								&nbsp;&nbsp;[
								<a href="/biomarkers/removeStudyDataResource/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $study['id']?>/<?php echo $resource['id']?>">
									Remove this association
								</a>]<br/>
		...
		<div id="addstudypub" class="addstudypub fadeOut">
			...
			<form action="/biomarkers/addOrganDataPub" method="POST">
				<input type="hidden" name="biomarker_id"  value="<?php echo $biomarker['Biomarker']['id']?>"/>
				<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
			...
			<?php foreach ($organData['Publication'] as $publication):?>
				<li>
					<div class="studypubsnippet">
						...
						&nbsp;[
						<a href="/biomarkers/removeOrganDataPub/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $publication['id']?>">
							Remove this association
						</a>
						]<br/>
		...
		<div id="addstudyres" class="addstudyres fadeOut">
			...
			<form action="/biomarkers/addOrganDataResource" method="POST" style="margin-top:5px;">
				<input type="hidden" name="biomarker_id"  value="<?php echo $biomarker['Biomarker']['id']?>"/>
				<input type="hidden" name="organ_data_id" value="<?php echo $organData['OrganData']['id']?>"/>
		...
		<ul class="displayList">
			<?php foreach ($organData['OrganDataResource'] as $resource):?>
				<li><div class="studyressnippet">
					...
					&nbsp;&nbsp;[
					<a href="/biomarkers/removeOrganDataResource/<?php echo $biomarker['Biomarker']['id']?>/<?php echo $organData['OrganData']['id']?>/<?php echo $resource['id']?>">
						Remove this association
					</a>
					]<br/>
...
<form method="POST" action="/biomarkers/addOrganData">
	<input type="hidden" name="biomarker_id" value="<?php echo $biomarker['Biomarker']['id']?>"/>
	<ul>
...
<script type="text/javascript">
	...
	// and this function let's us still call the jQuery code with $. 
		...
		$('#term-search').each(function() {
			$(this).autocomplete({
				source: '/terms/getAutocompleteTerms',
				select: function(event, ul) {
					var studyName = ul.item.value.split('|')[0];
					var studyId = ul.item.value.split('|')[1];
