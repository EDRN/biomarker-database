...
function Show($Render=false) {
	...
	if (($this->_Mode==0) and (($Render & TBS_EXIT)==TBS_EXIT)) {return $Ok;}
...
// Mode = 3 : GetBlockSource => save $Loc->BlockSrc with TBS Def tags
	...
	if ($Mode===1) {
		...
		$beg = $Loc->PosBeg;
		$end = $Loc->PosEnd;
		if (tbs_Locator_EnlargeToTag($Txt,$Loc,$Block,false)===false) return $this->meth_Misc_Alert($Loc,'at least one tag corresponding to '.$Loc->PrmLst['block'].' is not found. Check opening tags, closing tags and embedding levels.',false,'in block\'s definition');
		...
		if ($LocA->BlockFound) {
			...
			if ($DelField) {
				...
				if ($LocA->PosBeg2===false) {
					if (tbs_Locator_EnlargeToTag($Txt,$LocA,$LocA->PrmLst['block'],false)===false) $this->meth_Misc_Alert($LocA,'at least one tag corresponding to '.$LocA->PrmLst['block'].' is not found. Check opening tags, closing tags and embedding levels.',false,'in block\'s definition');
				} else {
